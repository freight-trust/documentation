# JSON Event Format for ChainEvents - Version 1.0


## Abstract

The JSON Format for ChainEvents defines how events are expressed in JavaScript
...
[ChainEvents][ce] is a standardized and protocol-agnostic definition of the
structure and metadata description of events. This specification defines how the
elements defined in the ChainEvents specification are to be represented in the
JavaScript Object Notation (JSON) Data Interchange Format ([RFC8259][rfc8259]).

The [Attributes](#2-attributes) section describes the naming conventions and
data type mappings for ChainEvents attributes.

The [Envelope](#3-envelope) section defines a JSON container for ChainEvents
...
This section defines how ChainEvents attributes are mapped to JSON. This
specification does not explicitly map each attribute, but provides a generic
mapping model that applies to all current and future ChainEvents attributes,
...
The core [ChainEvents specification][ce] defines a minimal abstract type system, that is dervied from the [Cloud Events](#cloud_events) reference specification. 

### 2.2. Type System Mapping

The [ChainEvents type system][ce-types] MUST be mapped to JSON types as follows,
...
standard outside of ChainEvents, with a formal JSON mapping, and there might be
...
for all other event formats that are part of the ChainEvents core at the time of
the submission or revision.

If required, like when decoding Maps, the ChainEvents type can be determined by
...
respective ChainEvents type when the mapping rules are fulfilled.

### 2.3. Examples

The following table shows exemplary attribute mappings:

| ChainEvents     | Type          | Exemplary JSON Value    |
| --------------- | ------------- | ----------------------- |
...
The ChainEvents [JSONSchema](http://json-schema.org) for the spec is located
[here](spec.json) and contains the definitions for validating events in JSON.

## 3. Envelope

Each ChainEvents event can be wholly represented as a JSON object.

Such a representation MUST use the media type `application/Chainevents+json`.
...
mutually exclusive in a JSON serialized ChainEvent.

When a ChainEvents is deserialized from JSON, the presence of the `data_base64`
...
In the _JSON Batch Format_ several ChainEvents are batched into a single JSON
document. The document is a JSON array filled with ChainEvents in the [JSON
...
### 4.1. Mapping ChainEvents

This section defines how a batch of ChainEvents is mapped to JSON.

The outermost JSON element is a [JSON Array][json-array], which contains as
elements ChainEvents rendered in accordance with the [JSON event
format][json-format] specification.

### 4.2. Envelope

A JSON Batch of ChainEvents MUST use the media type
`application/Chainevents-batch+json`.

### 4.3. Examples

An example containing two ChainEvents: The first with `Binary`-valued data, the
...
An example of an empty batch of ChainEvents (typically used in a response, but
